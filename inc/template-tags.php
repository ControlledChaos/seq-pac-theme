...
if ( ! function_exists( 'spr_theme_posted_on' ) ) :
	...
	function spr_theme_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'seq-pac-theme' ),
...
if ( ! function_exists( 'spr_theme_entry_footer' ) ) :
	...
	function spr_theme_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'seq-pac-theme' ) );
			if ( $categories_list ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'seq-pac-theme' ) . '</span>', $categories_list );
			}

			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'seq-pac-theme' ) );

			if ( $tags_list ) {
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'seq-pac-theme' ) . '</span>', $tags_list );
			}
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'seq-pac-theme' ),
						array(
							'span' => array(
								'class' => array(),
							),
